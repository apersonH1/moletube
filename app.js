...
// ✅ Shorts endpoint — nocookie-only, logs fresh ones if low count
app.get("/api/shorts", async (req, res) => {
  let logs = await readLogs();
  const nocookieShorts = logs.filter(v => v.url.includes("youtube-nocookie.com"));

  // If fewer than 10 saved, fetch more from YouTube
  if (nocookieShorts.length < 10) {
    const randomWords = ["funny", "music", "tech", "animals", "sports", "games"];
    const query = randomWords[Math.floor(Math.random() * randomWords.length)];
    const ytUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoDuration=short&maxResults=10&q=${query}&key=${YT_API_KEY}`;
  ...
  // Send 10 latest nocookie videos (random order)
...
app.listen(3000, () => console.log("✅ Moletube backend running on port 3000"));
